module UsersHelper
  ...
  def friendship_requested?(_user)
    Friendship.where(user_id: id, friend_id: user_id, confirmed: nil)
  end

  def friend_or_friendship_requested(user, current_user)
    unless current_user.friendship_requested?(user) || current_user.friend?(user) ||
